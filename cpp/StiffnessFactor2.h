...
class StiffnessFactor2: public gtsam::NoiseModelFactor5<gtsam::Pose3, gtsam::Pose3, gtsam::Pose3, gtsam::Pose3, gtsam::Vector6> {
...
public:
  ...
  StiffnessFactor2(gtsam::Key key1, gtsam::Key key2, gtsam::Key key3, gtsam::Key key4, gtsam::Key key5, bool zeroJac) :
  ...
  gtsam::Vector evaluateError(const gtsam::Pose3& p1, const gtsam::Pose3& p2,
    ...
    boost::optional<gtsam::Matrix&> H5 = boost::none) const {
      ...
      gtsam::Vector s = (gtsam::Vector6() << v[0], pow(v[1],-0.5)*v[0], pow(v[2],-0.5)*v[0], pow(v[3],-0.5)*v[5], pow(v[4],-0.5)*v[5], v[5]).finished();
      /*
